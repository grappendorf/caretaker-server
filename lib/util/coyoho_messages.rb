module CoYoHoMessages

	COYOHO_VERSION ||= 1

	# Message types bits 6..7
	COYOHO_MESSAGE_REQUEST ||= 0 << 6
	COYOHO_MESSAGE_RESPONSE ||= 2 << 6
	COYOHO_MESSAGE_NOTIFY ||= 3 << 6
	COYOHO_MESSAGE_TYPE_MASK ||= 3 << 6
	COYOHO_MESSAGE_COMMAND_MASK ||= 0x3f

	# Commands bits 0..5
	COYOHO_RESET ||= 0
	COYOHO_ADD_LISTENER ||= 1
	COYOHO_REMOVE_LISTENER ||= 2
	COYOHO_PROGRAM_WRITE ||= 3
	COYOHO_PROGRAM_READ ||= 4
	COYOHO_SWITCH_WRITE ||= 5
	COYOHO_SWITCH_READ ||= 6
	COYOHO_SENSOR_READ ||= 7
	COYOHO_SERVO_WRITE ||= 8
	COYOHO_SERVO_READ ||= 9
	COYOHO_PWM_WRITE ||= 10
	COYOHO_PWM_READ ||= 11
	COYOHO_RGB_WRITE ||= 12
	COYOHO_RGB_READ ||= 13
	COYOHO_DUMP ||= 14
	COYOHO_REFLOW_OVEN_ACTION ||= 15
	COYOHO_REFLOW_OVEN_STATUS ||= 16

	# Value write modes
	COYOHO_WRITE_DEFAULT ||= 0
	COYOHO_WRITE_ABSOLUTE ||= 1
	COYOHO_WRITE_INCREMENT ||= 2
	COYOHO_WRITE_INCREMENT_DEFAULT ||= 3
	COYOHO_WRITE_DECREMENT ||= 4
	COYOHO_WRITE_DECREMENT_DEFAULT ||= 5
	COYOHO_WRITE_TOGGLE ||= 6

	# Sensor types
	COYOHO_SENSOR_TEMPERATURE ||= 0
	COYOHO_SENSOR_BRIGHTNESS ||= 1
	COYOHO_SENSOR_SERVO ||= 2
	COYOHO_SENSOR_POWER_CONSUMPTION ||= 3
	COYOHO_SENSOR_ALL ||= 255

	# Servo types
	COYOHO_SERVO_AZIMUTH ||= 0
	COYOHO_SERVO_ALTITUDE ||= 1
	COYOHO_SERVO_ALL ||= 255

	# Dump values
	COYOHO_DUMP_VERSION	||= 0
	COYOHO_DUMP_LISTENER ||= 1

	# Reflow oven actions
	COYOHO_REFLOW_OVEN_OFF ||= 0
	COYOHO_REFLOW_OVEN_START ||= 1
	COYOHO_REFLOW_OVEN_COOL ||= 2

	# Reflow oven modes
	COYOHO_REFLOW_OVEN_MODE_OFF ||= 0
	COYOHO_REFLOW_OVEN_MODE_REFLOW ||= 1
	COYOHO_REFLOW_OVEN_MODE_MANUAL ||= 2
	COYOHO_REFLOW_OVEN_MODE_COOL ||= 3

	# Reflow oven states
	COYOHO_REFLOW_OVEN_STATE_IDLE	||= 0
	COYOHO_REFLOW_OVEN_STATE_ERROR ||= 1
	COYOHO_REFLOW_OVEN_STATE_SET ||= 2
	COYOHO_REFLOW_OVEN_STATE_HEAT ||= 3
	COYOHO_REFLOW_OVEN_STATE_PRECOOL ||= 4
	COYOHO_REFLOW_OVEN_STATE_PREHEAT ||= 5
	COYOHO_REFLOW_OVEN_STATE_SOAK ||= 6
	COYOHO_REFLOW_OVEN_STATE_REFLOW ||= 7
	COYOHO_REFLOW_OVEN_STATE_REFLOW_COOL ||= 8
	COYOHO_REFLOW_OVEN_STATE_COOL ||= 9
	COYOHO_REFLOW_OVEN_STATE_COMPLETE ||= 10

end
